...
%     end
    ...
    for iT = 1:numTFilesToUse
        ...
        for iSession = 1:numSessions
            ...
            %ti = session.getTrialsToUse();
            if session.getNumTrialsToUse() ~= 1
                error('Session should have only 1 trial to process (but has %d)!', session.getNumTrialsToUse());
            end
            trial = session.getTrialToUse(1);
            trialId = trial.getTrialId(); % could be the second actual trial if trial 1 was marked as not to use
